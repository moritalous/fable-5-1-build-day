...
<Aside type="note" title="About per-workspace limits">
	The console also lets you set limits per workspace. If you want a separate limit for the workspace you created in [Creating an API key](/en/preparation/apikey/), you can set one from that workspace's settings. If you don't set one, the organization-wide limit above applies as-is.

	(You can't set this per-workspace limit on the `Default` workspace.)
</Aside>
